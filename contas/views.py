    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if PerfilUsuario.objects.filter(email=email).exists():
                return Response({'erro': _('Email já cadastrado')}, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                return Response({'erro': _('Falha ao enviar email de ativação.')}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

            return Response({'sucesso': _('Usuário cadastrado com sucesso. Verifique seu email para ativar a conta.')}, status=status.HTTP_201_CREATED)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if usuario is not None and usuario.email_confirmed:
                ...
                return Response({'error': _('Email não confirmado. Por favor, ative sua conta.')}, status=status.HTTP_401_UNAUTHORIZED)
            else:
                return Response({'error': _('Email ou senha inválidos.')}, status=status.HTTP_401_UNAUTHORIZED)
...
class AtivacaoContaView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            confirmacao_email = AtivacaoConta.objects.filter(codigo_ativacao=codigo_ativacao).first()

            if confirmacao_email:
                if confirmacao_email.verificar_confirmacao(codigo_ativacao):
                    return Response({'success': _('Conta Ativada. Prossiga para o Login')}, status=status.HTTP_200_OK)
                else:
                    return Response({'error': _('Código de confirmação inválido.')}, status=status.HTTP_400_BAD_REQUEST)
            else:
                return Response({'error': _('Código de confirmação inválido.')}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response({'error': _('Usuário com este email não existe.')}, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                return Response({'error': _('Falha ao enviar email de redefinição.')}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

            return Response({'success': _('Código de verificação enviado com sucesso.')}, status=status.HTTP_200_OK)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response({'error': _('Código de verificação inválido.')}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({'success': _('Senha redefinida com sucesso.')}, status=status.HTTP_200_OK)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if new_email != user.email:
                raise PermissionDenied("O email fornecido não corresponde ao email do usuário logado.")

            # Envia o email de confirmação de alteração com o código
            self.enviar_confirmacao_alteracao_email(user)

            return Response({'success': 'Solicitação de alteração de email enviada com sucesso.'}, status=status.HTTP_200_OK)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if user.email_verification_code == code:
                ...
                user.save()
                return Response({'success': 'Email alterado com sucesso.'}, status=status.HTTP_200_OK)
            else:
                return Response({'error': 'Código de verificação inválido ou expirado.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if not user.check_password(old_password):
                return Response({'error': _('Senha atual incorreta.')}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response({'erro': _('Usuário não encontrado.')}, status=status.HTTP_404_NOT_FOUND)

            if usuario.email_confirmado:
                return Response({'erro': _('Esta conta já está ativada.')}, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                return Response({'erro': _('Falha ao enviar email com novo código de ativação.')}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

            return Response({'sucesso': _('Novo código de ativação enviado com sucesso. Verifique seu email.')}, status=status.HTTP_200_OK)
